...
- **Location**: `packages/adt-mcp/src/lib/tools/sap-connect.ts`
- **The Conflict**: The tool imports `getAdtClientV2Safe` directly from `@abapify/adt-cli`. While `adt-mcp` is allowed to depend on `adt-cli`, tools are ideally supposed to depend on the _service layer_, not internal utility functions.
